...
@section('title', 'Edit Timeframe')

@section('content')
<div class="bg-white shadow-md rounded-lg p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold">Edit Timeframe</h1>
            <p class="text-gray-600 mt-1">Modify timeframe details</p>
        </div>
    ...
    <form action="{{ route('coordinator.timeframes.update', $timeframe->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="academic_year" id="academic_year" 
                       ...
                       value="{{ old('academic_year', $timeframe->academic_year) }}">
            ...
            <div>
                ...
                <select name="semester" id="semester" 
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="1" {{ old('semester', $timeframe->semester) == '1' ? 'selected' : '' }}>Semester 1</option>
                    <option value="2" {{ old('semester', $timeframe->semester) == '2' ? 'selected' : '' }}>Semester 2</option>
                    <option value="3" {{ old('semester', $timeframe->semester) == '3' ? 'selected' : '' }}>Semester 3</option>
                </select>
                @error('semester')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="start_date" id="start_date" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       required value="{{ old('start_date', $timeframe->start_date->format('Y-m-d')) }}">
                @error('start_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="end_date" id="end_date" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       required value="{{ old('end_date', $timeframe->end_date->format('Y-m-d')) }}">
                @error('end_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="proposal_submission_deadline" id="proposal_submission_deadline" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       required value="{{ old('proposal_submission_deadline', $timeframe->proposal_submission_deadline->format('Y-m-d\TH:i')) }}">
            ...
            <div>
                ...
                <input type="datetime-local" name="supervisor_confirmation_deadline" id="supervisor_confirmation_deadline" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       required value="{{ old('supervisor_confirmation_deadline', $timeframe->supervisor_confirmation_deadline->format('Y-m-d\TH:i')) }}">
            ...
            <div>
                ...
                <input type="number" name="max_applications_per_student" id="max_applications_per_student" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       min="1" max="10" value="{{ old('max_applications_per_student', $timeframe->max_applications_per_student) }}" required>
            ...
            <div>
                ...
                <input type="number" name="max_appointments_per_student" id="max_appointments_per_student" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                       min="1" max="10" value="{{ old('max_appointments_per_student', $timeframe->max_appointments_per_student) }}" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="status" id="status" 
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="draft" {{ old('status', $timeframe->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                    <option value="active" {{ old('status', $timeframe->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="completed" {{ old('status', $timeframe->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="archived" {{ old('status', $timeframe->status) == 'archived' ? 'selected' : '' }}>Archived</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
