...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form action="{{ route('coordinator.userRegister') }}" method="GET" class="flex gap-4"> <!-- Search and filter the users -->
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form action="{{ route('coordinator.register.store') }}" method="POST" enctype="multipart/form-data"> <!-- Form to upload the CSV file -->
            @csrf
            <div class="flex gap-4">
                <div class="flex-1">
                    <input type="file"
                           name="csv_file"
                           accept=".csv"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"> <!-- Box that show name of the file -->
                </div>
    ...
    <!-- Messages -->
    @if(session()->has('success')) <!-- Display the success message of uploading the CSV file -->
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4">
            {{ session('success') }}
        </div>
    @endif

    @if(session()->has('error')) <!-- Display the error message of uploading the CSV file -->
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4">
            {{ session('error') }}
        </div>
    @endif

    @if(session()->has('failures')) <!-- Display the error message of registering the users -->
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($users as $user) <!-- Display the users, fetched from the table user -->
                <tr>
    ...
    <div class="mt-4">
        @if ($users->hasPages())
            <nav role="navigation" aria-label="Pagination Navigation" class="flex justify-center">
                ...
                @else
                    <a href="{{ $users->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-l-md hover:bg-gray-50">
                        Previous
                    </a>
                @endif
                ...
                @if ($users->hasMorePages())
                    <a href="{{ $users->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-r-md hover:bg-gray-50">
                        Next
                    </a>
                @else
